...
Router.route('/phaseDonsReprise', function () {
        this.layout('phaseDonsTemplate');
        this.render('phaseDonsTemplate');
        const donsEnCours = DonsGorgees.findOne({donsEnCours: true});
        if (donsEnCours !== undefined) {
            Router.go("phaseDons", {
                rangCarteShort: donsEnCours.carte.rank.shortName,
                rangCarteLong: donsEnCours.carte.rank.longName,
                couleurCarte: donsEnCours.carte.suit.name
            });
        }
    },
    {
        name: 'phaseDonsReprise'
    }
);
